...
class TestLogo:
    ...
    @allure.title('Проверка, что при клике на логотип Самокат, происходит переход на главную страницу Самоката')
    def test_if_click_scooter_logo_main_page_opens(self, logo_page):
        logo_page.click_order_button()
        logo_page.click_scooter_logo()
        assert logo_page.check_opened_page() == QA_SCOOTER_URL


    @allure.title('Проверка, что при клике на логотип Яндекс, происходит переход на главную страница Дзена')
